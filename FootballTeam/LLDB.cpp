...
template <typename T>
void LLDB<T>::push_fr(node** head, T x)
{
...
template <typename T>
void LLDB<T>::push_b(node** head, T x)
{
    node *new_node = makenode(x);
    if (*head == NULL)
    {
...
template <typename T>
void LLDB<T>::insert(node **head, int k, T x)
{
